...
#' @param x character vector, or a list that can be unlisted to a character
#'   vector.
...
char_count <- function(x){
    ...
    if (!is.character(chars))
        stop("Input must a character vector or a list that can be unlisted to a character vector.")
